	...
	<view class="order">
		...
		<view class="miniOrder">
			<view class="userInfo" v-if="userInfo">
				...
				<view class="user_status">
					...
					<text v-if="orderList">起名订单记录共{{orderList.length||'0'}}条</text>
				</view>
				<view class="refresh">
					<image src="../../static/refresh.png" mode="" @click="refreshHandler" :style="{transform: 'rotate('+refresh+'deg)'}"></image>
				</view>
			</view>
			<view v-else class="mt40">
				<button  type="primary" open-type="getUserInfo" @getuserinfo="loginAuth">授权登陆</button>
			</view>
		</view>
		...
		<view class="order_list" v-if="orderList&&orderList.length>0">
			<view class="order_item" v-for="(item,i) in orderList" :key="i">
				...
				<view class="content">
					<view class="">
						<rich-text class="content_title" :nodes="item.title"></rich-text><text class="money">￥{{item.money}}</text>
					</view>
				</view>
